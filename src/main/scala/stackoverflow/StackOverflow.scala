...
import stackoverflow.StackOverflow.printResults
...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(ss => (firstLangInTag(ss._1.tags, langs), ss._2)).
      filter(_._1.isDefined).map(ss => (ss._1.get * langSpread, ss._2)).
      persist
  }
